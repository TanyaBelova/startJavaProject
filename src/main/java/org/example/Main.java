...
public class Main {
    ...
    public static void jsonDeserialize() throws IOException {
        ...
        List<Book> booksFromJsonList = new ObjectMapper().readValue(
    ...
    public static void jsonSerialize(ArrayList<Book> bookList) throws JsonProcessingException {
        ...
        String jsonBookList = mapper.writeValueAsString(bookFilter(bookList, "Английский"));
        System.out.println(jsonBookList);
    }

    public static List<Book> bookFilter(ArrayList<Book> bookList, String language) {
        return bookList.stream().filter(book -> book.getLanguage().equals(language)).collect(Collectors.toList());
    }
